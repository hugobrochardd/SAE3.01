...
class ProductCollection {
    ...
    async load(uri){
        ...
        for(let item of objects){
            let p = new Product(item.id_product, item.name, item.id_category, item.price, item.delivery, item.description, item.category);
            this.#add(p);
        }
    ...
    async create(name, idcat){
        ...
        if (object){
            this.#add(new Product(object.id_product, object.name, object.id_category, object.price, object.delivery, object.description, object.category));
        }
        else{
    ...
    findByCategory(idcat){
        return this.#products.filter( p => p.getId_category()==idcat);
    }
}
